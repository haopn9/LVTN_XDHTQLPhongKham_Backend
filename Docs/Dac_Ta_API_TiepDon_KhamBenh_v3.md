...
_Bản cập nhật theo nghiệp vụ mới - Luồng khám TRỰC TIẾP tại phòng khám (v3 - bổ sung Kê vật tư)_
...
| 14    | \[MỚI\] Bổ sung bước "Kê vật tư" vào quy trình khám bệnh - chỉ áp dụng cho phiếu CÓ chỉ định CLS, kê SAU bước kê thuốc, TRƯỚC bước kết luận khám | Cao        | 42, 43              |
| 15    | \[MỚI\] Kê vật tư dùng chung cơ chế kiểm tra & trừ tồn kho theo lô (LoVatTu) theo nguyên tắc FEFO, chặn cứng nếu không đủ tồn - giống hệt cơ chế của kê đơn thuốc | Cao        | 43                  |
| 16    | \[MỚI\] Kê vật tư dùng cơ chế CỘNG DỒN (không REPLACE như đơn thuốc) - mỗi lần gọi API chỉ trừ kho thêm theo danh sách gửi lên, không xoá/hoàn kho dòng đã kê trước đó | Cao        | 43                  |
...
**1.6 \[MỚI\] Bảng DanhMucVatTu / LoVatTu / ChiTietVatTuPhieuKham**

DanhMucVatTu: MaVatTu, TenVatTu, QuyCach, DonViTinh, IsActive

LoVatTu: MaLo, MaVatTu, MaNcc, SoLuongNhap, SoLuongTon,

GiaNhap, GiaBan, NgaySanXuat, HanSuDung (NOT NULL)

ChiTietVatTuPhieuKham: MaPhieu, MaVatTu, SoLuong, DonGia (PK ghép MaPhieu + MaVatTu)

- Cấu trúc vật tư SONG SONG với cấu trúc thuốc (DanhMucThuoc / LoThuoc), dùng chung nguyên tắc trừ kho theo lô - FEFO. Khác biệt quan trọng so với thuốc:
- KHÔNG có bảng "đơn vật tư" (DonVatTu) làm header như DonThuoc - ChiTietVatTuPhieuKham gắn trực tiếp vào MaPhieu, không qua bảng trung gian.
- ChiTietVatTuPhieuKham có sẵn cột DonGia (NOT NULL) - khác ChiTietDonThuoc không lưu đơn giá. Khi FEFO phải trừ qua NHIỀU lô có GiaBan khác nhau cho cùng 1 MaVatTu, DonGia lưu vào ChiTietVatTuPhieuKham LẤY THEO GIÁ CỦA LÔ ĐẦU TIÊN bị trừ (lô có HanSuDung gần nhất).
- PK của ChiTietVatTuPhieuKham chỉ là (MaPhieu, MaVatTu) - KHÔNG lưu vết MaLo đã trừ. Do đó API kê vật tư dùng cơ chế CỘNG DỒN (không REPLACE): mỗi lần gọi PUT api/KhamBenh/{maPhieu} với nhóm vatTuList, hệ thống CỘNG THÊM số lượng vào dòng ChiTietVatTuPhieuKham đã có (nếu trùng MaVatTu) hoặc INSERT dòng mới, và trừ kho LoVatTu thêm theo FEFO trên tồn kho hiện tại - KHÔNG xoá dòng cũ, KHÔNG hoàn kho.
- Hệ quả: API kê vật tư KHÔNG hỗ trợ "sửa/xoá vật tư đã kê" - nếu bác sĩ kê nhầm, cần cơ chế điều chỉnh tồn kho riêng do bộ phận Kho xử lý (ngoài phạm vi đặc tả này - xem mục 6.3).
- Kê vật tư CHỈ được phép khi phiếu khám có ít nhất 1 chỉ định CLS (bảng DichVuYTe có dòng ứng với MaPhieu này) - phiếu không có CLS thì không hiển thị/không cho gọi bước này.

**1.7 Bảng NhanVien / Users / Roles**
...
B8b: \[MỚI\] Lấy danh sách ChiTietVatTuPhieuKham JOIN DanhMucVatTu theo MaPhieu (nếu phiếu không có vật tư nào thì trả mảng rỗng)
...
// \[MỚI\] chỉ có dữ liệu khi phiếu có chỉ định CLS; phiếu không có CLS → mảng rỗng

"danhSachVatTu": \[

{ "maVatTu": &lt;string&gt;, "tenVatTu": &lt;string&gt;, "donViTinh": &lt;string&gt;,

"soLuong": &lt;int&gt;, "donGia": &lt;decimal(18,2)&gt; }
...
**\[MỚI\]** danhSachVatTu lấy trực tiếp từ ChiTietVatTuPhieuKham - chỉ phát sinh dữ liệu ở các phiếu có chỉ định CLS; đây là dữ liệu CỘNG DỒN qua các lần kê (xem mục 43), không phải "1 lần kê = 1 dòng" mà là tổng số lượng đã kê cho từng MaVatTu.
...
| Kê vật tư \[MỚI\] | Không có                                          | Thêm nhóm vatTuList - chỉ cho phép khi phiếu có CLS; bắt buộc kiểm tra & trừ LoVatTu theo FEFO giống thuốc; cơ chế CỘNG DỒN (không REPLACE) |
...
\-- Nhóm 4b: Kê vật tư \[MỚI\] (CỘNG DỒN - chỉ trừ kho thêm, không xoá/không hoàn kho dòng cũ) --

\- vatTuList &lt;array&gt; : Danh sách vật tư cần kê thêm - Tuỳ chọn; CHỈ được gửi nếu phiếu đang có ít nhất 1 chỉ định CLS

\- maVatTu &lt;string&gt; : Mã vật tư (FK DanhMucVatTu) - BẮT BUỘC nếu có

\- soLuong &lt;int&gt; : Số lượng cần kê thêm (số nguyên dương) - BẮT BUỘC nếu có
...
Ví dụ Request (Kê vật tư - chỉ áp dụng khi phiếu có CLS):

PUT api/KhamBenh/PK_260609_001

{

"vatTuList": \[ { "maVatTu": "VT001", "soLuong": 2 }, { "maVatTu": "VT005", "soLuong": 1 } \]
...
B8b: \[Nhóm 4b - Kê vật tư, MỚI: kiểm tra + trừ kho FEFO, CỘNG DỒN\] Nếu body có vatTuList:

\- \[MỚI\] Chặn cứng nếu phiếu KHÔNG có bất kỳ dòng DichVuYTe nào (chưa từng chỉ định CLS)

→ từ chối (409) - kê vật tư chỉ áp dụng cho phiếu có CLS

\- Với danh sách vatTuList gửi lên, với TỪNG maVatTu:

(a) Validate maVatTu tồn tại và IsActive = 1 trong DanhMucVatTu

(b) Tính tồn khả dụng = SUM(LoVatTu.SoLuongTon) của các lô CHƯA hết hạn

(HanSuDung >= GETDATE()) của maVatTu đó

(c) Nếu tồn khả dụng < soLuong yêu cầu → CHẶN CỨNG, từ chối cả request (409)

(d) Nếu đủ → trừ theo FEFO: sắp lô theo HanSuDung tăng dần, trừ dần từng lô

cho đến khi đủ soLuong (có thể trừ qua nhiều lô); ghi nhận GiaBan của LÔ ĐẦU TIÊN

bị trừ (lô hạn dùng gần nhất) để dùng làm DonGia ở bước (e)

(e) \[MỚI - CỘNG DỒN\] Nếu ChiTietVatTuPhieuKham đã có dòng (MaPhieu, maVatTu) →

UPDATE cộng thêm SoLuong (SoLuong = SoLuong + soLuong mới); DonGia GIỮ NGUYÊN

giá trị đã lưu ở lần kê đầu tiên (không ghi đè lại DonGia ở các lần cộng dồn sau)

Nếu chưa có dòng nào → INSERT mới (MaPhieu, maVatTu, SoLuong = soLuong,

DonGia = giá lô đầu tiên bị trừ ở bước (d))

\- \[MỚI\] KHÔNG xoá, KHÔNG hoàn kho bất kỳ dòng vật tư nào đã kê trước đó - API này

chỉ cộng thêm, không hỗ trợ sửa/xoá vật tư đã kê qua endpoint này

\- \[MỚI\] Toàn bộ (a)-(e) chạy trong CÙNG 1 transaction - nếu bất kỳ maVatTu nào

không đủ tồn, ROLLBACK toàn bộ, không kê một phần
...
**\[MỚI\]** Kê vật tư CHỈ áp dụng cho phiếu có chỉ định CLS (đã tồn tại ít nhất 1 dòng DichVuYTe cho phiếu); phiếu không có CLS mà gửi vatTuList → từ chối (409).

**\[MỚI\]** Kê vật tư: BẮT BUỘC đủ tồn kho (SUM SoLuongTon các lô LoVatTu chưa hết hạn) mới cho kê; không có chế độ ghi nhận thiếu hụt. Trừ kho theo FEFO, cùng transaction với việc cập nhật ChiTietVatTuPhieuKham.

**\[MỚI\]** Kê vật tư dùng cơ chế CỘNG DỒN: gọi lại API nhiều lần với cùng maVatTu sẽ cộng dồn SoLuong vào dòng đã có, DonGia giữ nguyên theo lần kê đầu tiên; KHÔNG xoá/không hoàn kho các lần kê trước. API này không hỗ trợ sửa hoặc xoá vật tư đã kê.

- Mã vật tư (maVatTu) phải tồn tại trong DanhMucVatTu và IsActive = 1.
...
- \[MỚI\] Kê vật tư khi phiếu chưa có chỉ định CLS nào → HTTP 409 | "Phiếu khám chưa có chỉ định CLS. Chỉ được kê vật tư cho phiếu có CLS!"
- \[MỚI\] Mã vật tư không tồn tại/ngưng sử dụng → HTTP 400 | "Vật tư không tồn tại hoặc đã ngừng sử dụng. Vui lòng kiểm tra lại"
- \[MỚI\] Số lượng vật tư không hợp lệ → HTTP 400 | "Số lượng vật tư phải là số nguyên dương. Vui lòng nhập lại"
- \[MỚI\] Không đủ tồn kho cho 1 hoặc nhiều vật tư → HTTP 409 | "Vật tư '{tenVatTu}' không đủ tồn kho (còn {soLuongTon}, yêu cầu {soLuong}). Vui lòng kiểm tra lại hoặc liên hệ kho!"
...
- \[MỚI\] Cơ chế sửa/xoá/hoàn kho vật tư đã kê nhầm - do PK ChiTietVatTuPhieuKham không lưu vết theo lô nên đợt này chỉ hỗ trợ CỘNG DỒN; điều chỉnh sai sót (nếu có) sẽ do bộ phận Kho xử lý thủ công/qua module Kho riêng.
